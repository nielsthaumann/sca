...
Spike density component analysis (SCA) decomposes EEG or MEG waveforms into neural components modeled with temporal probability density functions (with the <a href="https://github.com/nielsthaumann/sca/blob/master/runsca.m">runsca.m</a> function). </br></br>
Statistical thresholding of individual-level evoked responses can be performed with a half-split average consistency (SCA-HSAC) procedure [runsca_hsac.m](https://github.com/nielsthaumann/sca/blob/master/runsca_hsac.m), a more conservative statistical thresholding procedure with [runsca_stats.m](https://github.com/nielsthaumann/sca/blob/master/runsca_stats.m), or a simple template match (SCA-TM) approach [runsca_tm.m](https://github.com/nielsthaumann/sca/blob/master/runsca_tm.m). </br></br>
The [runsca_hsac.m](https://github.com/nielsthaumann/sca/blob/master/runsca_hsac.m) is recommended when the aim is to extract an evoked response of interest and filter out interfering neural components before performing within-participant and between-participant comparisons. [runsca_hsac.m](https://github.com/nielsthaumann/sca/blob/master/runsca_hsac.m) enables relatively a liberal suppression of interfering neural acitivity, with a relatively low risk of mistakenly filtering out the evoked responses of interest. </br></br>
The more conservative statistical thresholding is recommended with the <a href="https://github.com/nielsthaumann/sca/blob/master/runsca_stats.m">runsca_stats.m</a> function, when the aim is to test whether an evoked response is present (or not), since it applies a relatively conservative statistical test to identify the evoked response. </br>
...
Suppose some news breaks in the megacity. The exact time news reaches an individual is uncertain, but the arrival time varies systematically around a predicted time. </br>
    Similarly, the images below show white flashes at variable times of arrival during the influx of Ca2+ ions after 'neuronal spiking' have pushed out Na+ ions of neuron cell bodies as part of the electrochemical signaling cascade:
<br></br>
...
'Neuronal spiking' with systematic noise and influx of Ca2+ ions are a part of the electrochemical signaling cascade, where neurons trigger post-synaptic 'local field potentials' in neurons that receive a message. The post-synaptic 'local field potentials' from a large number (>10,000) of neurons are the main contributors to the brain activity measured with MEG/EEG: 
<br></br>
<p align="left">
...
A popular assumption is that brain activity measured with MEG/EEG is shaped as sine waves. However, in line with the observations of systematic noise in the timing of 'neuronal spiking', it turns out that the brain activity measured with MEG/EEG is more accurately modeled with systematic Gaussian noise in the timing of the activity than with sine waves ([Haumann et al., 2020](https://doi.org/10.1016/j.jneumeth.2020.108743)). 
<br>
</br>
